...
const GameHost: React.FC = () => {
  ...
  useEffect(() => {
    if (players.length > 0 && Object.keys(votes).length === players.length && !showResult && currentRound) {
  ...
  if (!gameState || !currentRound) return <div className="text-white text-center p-10">Loading game...</div>;
            ...
            {players.map((player, index) => (
              <motion.div 
                ...
                className={cn(
                  ...
                  index === 0 ? "bg-yellow-500/20 border-yellow-500/50" : "bg-white/5 border-white/10"
                )}
              >
                <div className="flex items-center gap-4">
                  <span className={cn(
                    "font-mono font-bold w-8 h-8 flex items-center justify-center rounded-full",
                    index === 0 ? "bg-yellow-500 text-black" : "bg-white/10 text-muted-foreground"
                  )}>
                    {index + 1}
                  </span>
    ...
    <GameLayout className="max-w-6xl">
      <div className="flex flex-col h-full gap-6">
        ...
        <div className="flex justify-between items-center">
          <div className="text-2xl font-bold">Round {currentRound.round_number} / {gameState.settings.rounds}</div>
          <div className={cn(
            "text-4xl font-black font-mono p-4 rounded-xl border-2",
            timeLeft <= 5 ? "text-red-500 border-red-500 animate-pulse" : "text-indigo-400 border-indigo-500"
          )}>
            {timeLeft}s
          </div>
          <div className="flex items-center gap-2">
            <Users className="w-6 h-6" />
            <span className="text-xl">{Object.keys(votes).length} / {players.length} Votes</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-[500px] relative">
           {/* Images Layer */}
           <div className={cn("grid grid-cols-2 gap-8 h-full transition-all duration-500", showResult ? "opacity-40 scale-95 blur-sm" : "opacity-100")}>
            {/* Image A */}
            <div className="relative group">
              <div className="absolute top-4 left-4 z-10 bg-black/50 px-4 py-2 rounded-lg text-2xl font-bold border border-white/20">A</div>
              <img 
                src={leftImage} 
                className={cn(
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex -space-x-4 z-20 px-4 py-2 bg-black/30 backdrop-blur-sm rounded-full border border-white/10 min-h-[60px] items-center justify-center">
                <AnimatePresence mode="popLayout">
                  {Object.entries(votes).filter(([_, choice]) => choice === 'A').map(([pid, _]) => {
            ...
            {/* Image B */}
            <div className="relative group">
              <div className="absolute top-4 left-4 z-10 bg-black/50 px-4 py-2 rounded-lg text-2xl font-bold border border-white/20">B</div>
              <img 
                src={rightImage} 
                className={cn(
                  "w-full h-full object-cover rounded-2xl border-4 transition-all duration-500",
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex -space-x-4 z-20 px-4 py-2 bg-black/30 backdrop-blur-sm rounded-full border border-white/10 min-h-[60px] items-center justify-center">
                <AnimatePresence mode="popLayout">
                  {Object.entries(votes).filter(([_, choice]) => choice === 'B').map(([pid, _]) => {
                    const player = players.find(p => p.id === pid);
                    if (!player) return null;
                    return (
                      <motion.div
          ...
          <AnimatePresence>
            {showResult && (
              ...
              >
                <Card className="w-full max-w-2xl bg-slate-900/90 border-indigo-500/50 backdrop-blur-xl shadow-2xl">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-6">
                    <div className="text-center space-y-2">
                      <h3 className="text-2xl font-bold">
                        Option <span className="text-green-400">{isRealLeft ? 'A' : 'B'}</span> was REAL!
                      </h3>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                      {players.map((player, index) => (
                        <div key={player.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
                          ...
                          <span className="font-mono font-bold text-indigo-300">{player.score} pts</span>
                        </div>
                      ))}
                    </div>
